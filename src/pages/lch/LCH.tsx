...
class BasicPlayer {
  ...
  attack(opponent: BasicPlayer) {
    if (this.boundingBox.intersectsBox(opponent.boundingBox)) {
      opponent.health -= 10
...
const Game = () => {
  ...
  const updatePositions = useCallback(() => {
    ...
    // 경계 상자 업데이트
    player1.updateBoundingBox()
    player2.updateBoundingBox()
  ...
  return (
    <div>
      ...
      <div onClick={() => navigate("/")}>Go to My one</div>
      <Canvas
        orthographic
...
const LCH = () => {
  return (
    <div>
      <h1>2D Fighter Game LCH PAGE</h1>
      <Game />
    </div>
  )
}
